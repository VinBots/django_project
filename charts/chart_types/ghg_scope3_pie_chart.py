...
def ghg_scope3_pie_chart_from_xls(company_id, params):
    ...
    scope3_upstream_colors = ['#ffbaba', '#ff7b7b','#ff5252','#ff0000','#a70000','#ff0000','#ff4d00','#ff7400']
    scope3_downstream_colors = ['#0000ff','#3232ff','#6666ff','#7f7fff','#9999ff','#ccccff','#e5e5ff']
    ...
    layout = go.Layout (    
        ...
        annotations=[dict(text='<b>Scope 3</b><br>Emissions<br>Y'+str(int(reporting_year)), x=0.5, y=0.5, font_size=20, showarrow=False)]
        )
    ...
    fig.update_traces(marker=dict(colors=scope3_colors, line=dict(color='#000000', width=2)))
