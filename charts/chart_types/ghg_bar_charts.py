...
def ghg_bar_chart_from_xls(company_id, params):
    ...
    if (
        len(
            df.loc[
                (df["ctrl_target_year"] == True) & (df["target_year"] > df["year_n"])
            ]
        )
        > 0
    ):
    ...
    if len(df) > 0:
        scope1_list = (
            df[["ghg_scope1_n-2", "ghg_scope1_n-1", "ghg_scope1_n"]].values[0].tolist()
        )
        scope2_list = (
            df[["ghg_scope2_n-2", "ghg_scope2_n-1", "ghg_scope2_n"]].values[0].tolist()
        )
        ...
        if is_target:
            ...
            trace_baseline = go.Bar(
                ...
                hovertemplate="<b>Scope:</b> 1-2<br>"
                + "<b>Year:</b> %{x}<br>"
                + "<b>Emissions:</b> %{y}",
            )
        ...
        trace_scope1 = go.Bar(
            ...
            hovertemplate="<b>Scope:</b> 1 (direct)<br>"
            + "<b>Year:</b> %{x}<br>"
            + "<b>Emissions:</b> %{y}",
        )
        ...
        layout = go.Layout(
            ...
            yaxis=dict(title="Emissions (in tons of CO<sub>2</sub>e)"),
            legend=dict(title="<b>Emissions</b>", orientation="v", traceorder="normal"),
            autosize=False,
