...
def corporates_search(request, corp_name=None):
    ...
    if check_validity(corp_name):
        ...
        xls_corp = {
            'ghg':
            get_ghg(company_id=selected_corp.company_id,
                    all_data=all_data['ghg_quant'],
                    reporting_year_data=all_data['reporting']),
            'score_data':
            get_score_data(company_id=selected_corp.company_id,
                           all_data=all_data['corp_scores']),
            'corp_scores_summary':
            get_scores_summary(company_id=selected_corp.company_id,
                               all_data=all_data['score_summary']),
            'corp_scores_details':
            get_scores_details(company_id=selected_corp.company_id,
                               all_data=all_data['score_details']),
            'targets':
            get_targets(company_id=selected_corp.company_id,
                        all_data=all_data['targets_quant']),
        }

        library_corp = get_library_data(company_id=selected_corp.company_id,
                                        all_data=all_data['library_db'])
        ...
        corp_data = {
            ...
            "selected_corp_bubble_chart": {
                'html':
                get_path_to_chart(selected_corp.company_id, "bubble"),
                'img':
                get_path_to_img(selected_corp.company_id, "bubble"),
                'exist':
                file_exist(get_path_to_img(selected_corp.company_id, "bubble"))
            },
            "selected_corp_ghg_bar_chart": {
                'html':
                get_path_to_chart(selected_corp.company_id, "ghg_bar"),
                'img':
                get_path_to_img(selected_corp.company_id, "ghg_bar"),
                'exist':
                file_exist(get_path_to_img(selected_corp.company_id,
                                           "ghg_bar"))
            },
            "selected_corp_ghg_scope3_pie_chart": {
                'html':
                get_path_to_chart(selected_corp.company_id, "ghg_pie_chart"),
                'img':
        ...
        return render(
            request, "django_project/corporates/home.html", {
...
def show_html(request, folder_name=None, file_name=None):
    ...
    path = os.path.join(BASE_DIR, DIR_TO_CORP_CHARTS_TEMPLATES, folder_name,
                        file_name)

    return render(request, "django_project/corporates/show_interactive.html", {
        "path": path,
    })
