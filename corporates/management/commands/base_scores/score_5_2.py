...
class Score_5_2(AbsBaseScore):
    ...
    def get_rating(self):
        ...
        valid_coverage = [Options.SCOPE123_LOC, Options.SCOPE123_MKT]

        ongoing_netzero_condition = Q(already_reached=Options.YES, ongoing=Options.YES)
        ongoing_coverage_condition = Q(
            ongoing_coverage__in=valid_coverage, ongoing_scope_3_coverage=Options.FULL
        )
        case1_queryset = NetZero.objects.filter(
            Q(company__company_id=self.company_score.company.company_id),
            ongoing_netzero_condition & ongoing_coverage_condition,
        )

        coverage_condition = Q(
            coverage__in=valid_coverage, scope_3_coverage=Options.FULL
        )
